defmodule SymphonyElixir.Linear.Client do
  ...
  @spec fetch_project_issues(pos_integer(), keyword()) :: {:ok, [Issue.t()]} | {:error, term()}
  def fetch_project_issues(limit, opts \\ []) when is_integer(limit) and limit > 0 and is_list(opts) do
    project_slug = Config.linear_project_slug()
    ...
    cond do
      ...
      true ->
        do_fetch_project_issues(project_slug, limit, request_fun)
    end
  end
  ...
  defp do_fetch_project_issues(project_slug, limit, request_fun) do
    do_fetch_project_issues_page(project_slug, limit, request_fun, nil, [])
  end

  defp do_fetch_project_issues_page(_project_slug, remaining, _request_fun, _after_cursor, acc_issues)
       when remaining <= 0 do
    {:ok, finalize_paginated_issues(acc_issues)}
  end

  defp do_fetch_project_issues_page(project_slug, remaining, request_fun, after_cursor, acc_issues) do
    page_size = Enum.min([remaining, @issue_page_size])
    ...
    with {:ok, body} <-
      ...
      remaining_after = remaining - length(issues)

      case next_page_cursor(page_info) do
        {:ok, next_cursor} ->
          do_fetch_project_issues_page(
            project_slug,
            remaining_after,
            request_fun,
            next_cursor,
