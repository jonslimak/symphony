...
tracker:
  ...
  terminal_states:
    ...
    - Done
workspace:
  root: ~/Projects/sym/workspaces-flow-variation-child-spawn
hooks:
  after_create: |
...
## Test-specific child-spawn rules

- This workflow is a child-spawn orchestration test, not autonomous task invention.
...
- When the issue is `In Progress`, perform only parent-stage execution:
  ...
  - if `Job 2` work is requested, leave it for `Agent Review` routing rather than doing it in the parent ticket
...
3. Route to the matching flow:
   ...
   - `In Progress` -> continue parent execution flow from current scratchpad comment.
   - `Agent Review` -> continue the review-routing flow from current scratchpad comment.
...
10. Update the workpad comment with final checklist status and validation notes.
    ...
    - Record whether the ticket includes `Job 1` only or `Job 1` plus `Job 2`.
    - Record whether the handoff artifact exists and is published.
...
2. Re-read the issue description and identify whether the downstream contract is:
   - `Job 1` only
   - explicit `Job 1` plus explicit `Job 2`
   - ambiguous or invalid
3. Verify the parent result needed for child-spawn routing:
   - `Job 1` is complete
   - the named handoff artifact exists
   - the named handoff artifact has a published branch or PR reference
...
5. If `Job 2` is absent, vague, or would require multiple downstream tickets, do not invent a child task.
   - Record the reason in the workpad.
...
## Completion bar before Agent Review / Human Review

- Step 1/2 checklist is fully complete and accurately reflected in the single workpad comment.
- Acceptance criteria and required ticket-provided validation items are complete for the parent scope.
